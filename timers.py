...
class Timer:
    ...
    def __init__(self, conf):
        ...
        self.relay = Pin(33, Pin.OUT)
        self.active = False
        if (self.timer_type == 'interval'):
            self.period = timer_minutes(conf['conf']['interval_on'])
            self.period_off = timer_seconds(conf['conf']['interval_off'])
